	...
	public void testTranslation() {
		scene.notUndoableChange(new ISceneOperation() {
			...
			public void perform(final Scene scene) {
				...
				final Vector3 r1 = node.localToWorld(new Vector3(0, 0, 0),
						null);
				final Vector3 r2 = node.localToWorld(new Vector3(3, 2, 1),
						null);
				...
				final Vector3 r3 = transformation.transformPoint(new Vector3(
						0, 0, 0));
				final Vector3 r4 = transformation.transformPoint(new Vector3(
						3, 2, 1));
	...
	public void testScale() {
		scene.notUndoableChange(new ISceneOperation() {
			...
			public void perform(final Scene scene) {
				...
				final Vector3 r1 = node.localToWorld(new Vector3(1, 1, 1),
						null);
				final Vector3 r2 = node.localToWorld(new Vector3(6, 3, 2),
						null);
		...
		for (int i = 0; i < 100; i++) {
			...
			scene.notUndoableChange(new ISceneOperation() {
				...
				public void perform(final Scene scene) {
					...
					final Vector3 r1 = node.localToWorld(
							new Vector3(1, 0, 0), null);
					final Vector3 r2 = node.localToWorld(
							new Vector3(0, 1, 0), null);
					final Vector3 r3 = node.localToWorld(
