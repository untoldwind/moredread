...
class EarClip {
	...
	/**
	 * Classifies all the internal angles of the loop referenced by ind. the
	 * following classification is used: 0 ... if angle is 180 degrees 1 ... if
	 * angle between 0 and 180 degrees 2 ... if angle is 0 degrees -1 ... if
	 * angle between 180 and 360 degrees -2 ... if angle is 360 degrees
	 */
	static void classifyAngles(final Triangulator triRef, final int ind) {
		int ind0, ind1, ind2;
		int i0, i1, i2;
		int angle;
	...
	static void classifyEars(final Triangulator triRef, final int ind) {
		int ind1;
		int[] ind0, ind2;
		double[] ratio;
	 ...
	 * handling.
	 */
	static boolean isEar(final Triangulator triRef, final int ind2,
			final int[] ind1, final int[] ind3, final double[] ratio) {
		int i0, i1, i2, i3, i4;
		int ind0, ind4;
		BBox bb;
		...
		// System.out.println("isEar :(1) convex " + convex + " coneOk " +
		// coneOk );
		...
		// System.out.println("isEar :(2) convex " + convex + " coneOk " +
		// coneOk );

		if (coneOk) {
			// check whether this diagonal is a valid diagonal. this translates
			// to
	 ...
	 */
	static boolean clipEar(final Triangulator triRef, final boolean[] done) {
		...
		// the length of the triangle's longest side divided by the length of
		// the
		...
		if (angle1 > 0) {
			if (isEar(triRef, ind1, index0, index2, ratio)) {
				...
				Heap.insertIntoHeap(triRef, ratio[0], ind1, index0[0],
						index2[0]);
			}
		}
		...
		if (angle3 > 0) {
			if (isEar(triRef, ind3, index2, index4, ratio)) {
				Heap.insertIntoHeap(triRef, ratio[0], ind3, index2[0],
						index4[0]);
			}
		}
