...
class Bridge {
	...
	static void constructBridges(Triangulator triRef, int loopMin, int loopMax) {
		...
		if (triRef.noHashingEdges != true)
			System.out
					.println("Bridge:constructBridges noHashingEdges is false");
		if (loopMax <= loopMin)
		...
		if (loopMax > triRef.numLoops)
			System.out
					.println("Bridge:constructBridges loopMax>triRef.numLoops");
		...
		// construct bridges. every bridge will eminate at the left-most point
		// of
		...
		for (j = 0; j < numLeftMost; ++j) {
			if (!findBridge(triRef, ind0[0], i0[0], triRef.leftMost[j].index,
					ind1, i1)) {
				// if (verbose)
				// fprintf(stderr,
	...
	/**
	 * We try to find a vertex i1 on the loop which contains i such that i1 is
	 * close to start, and such that i1, start is a valid diagonal.
	 */
	static boolean findBridge(Triangulator triRef, int ind, int i, int start,
		...
		triRef.distances[numDist].dist = Numerics.baseLength(
				triRef.points[start], triRef.points[i1[0]]);
		...
		for (j = 0; j < numDist; ++j) {
			...
			if (i1[0] <= start) {
				...
				coneOk = Numerics
						.isInCone(triRef, i0, i1[0], i2, start, convex);
				if (coneOk) {
					bb = new BBox(triRef, i1[0], start);
					if (!NoHash.noHashEdgeIntersectionExists(triRef, bb, -1,
							-1, ind1[0], -1))
						return true;
				}
			}
		...
		for (j = 0; j < numDist; ++j) {
			...
			i2 = triRef.fetchData(ind2);
			bb = new BBox(triRef, i1[0], start);
			if (!NoHash.noHashEdgeIntersectionExists(triRef, bb, -1, -1,
					ind1[0], -1))
				return true;
		}
	...
	static void insertBridge(Triangulator triRef, int ind1, int i1, int ind3,
			int i3) {
