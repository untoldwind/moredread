...
class Numerics {
	...
	static double min3(final double a, final double b, final double c) {
		return (((a) < (b)) ? (((a) < (c)) ? (a) : (c)) : (((b) < (c)) ? (b)
				: (c)));
	}

	static boolean lt(final double a, final double eps) {
		return ((a) < -eps);
	}

	static boolean le(final double a, final double eps) {
		return (a <= eps);
	}

	static boolean ge(final double a, final double eps) {
		return (!((a) <= -eps));
	}

	static boolean eq(final double a, final double eps) {
		return (((a) <= eps) && !((a) < -eps));
	}

	static boolean gt(final double a, final double eps) {
		return !((a) <= eps);
	}

	static double baseLength(final Vector2f u, final Vector2f v) {
		double x, y;
		x = (v).x - (u).x;
		y = (v).y - (u).y;
	...
	static double sideLength(final Vector2f u, final Vector2f v) {
		double x, y;
		x = (v).x - (u).x;
		y = (v).y - (u).y;
		return x * x + y * y;
	...
	/**
	 * This checks whether i3, which is collinear with i1, i2, is between i1,
	 * i2. note that we rely on the lexicographic sorting of the points!
	 */
	static boolean inBetween(final int i1, final int i2, final int i3) {
		return ((i1 <= i3) && (i3 <= i2));
	}

	static boolean strictlyInBetween(final int i1, final int i2, final int i3) {
		return ((i1 < i3) && (i3 < i2));
	}
	 ...
	 * in a consistent way.
	 */
	static double stableDet2D(final Triangulator triRef, final int i,
			final int j, final int k) {
		double det;
		Vector2f numericsHP, numericsHQ, numericsHR;

		// if((triRef.inPointsList(i)==false)||(triRef.inPointsList(j)==false)||
		// (triRef.inPointsList(k)==false))
		// System.out.println("Numerics.stableDet2D Not inPointsList " + i + " "
		// + j
		// + " " + k);
	 ...
	 * 
	 * @return +1 if the points i, j, k are given in CCW order; -1 if the points
	 *         i, j, k are given in CW order; 0 if the points i, j, k are
	 *         collinear.
	 */
	static int orientation(final Triangulator triRef, final int i, final int j,
			final int k) {
		int ori;
		double numericsHDet;
		numericsHDet = stableDet2D(triRef, i, j, k);
		// System.out.println("orientation : numericsHDet " + numericsHDet);
		if (lt(numericsHDet, triRef.epsilon)) {
			ori = -1;
		} else if (gt(numericsHDet, triRef.epsilon)) {
			ori = 1;
		} else {
			ori = 0;
		}
	 ...
	 */
	static boolean isInCone(final Triangulator triRef, final int i,
			final int j, final int k, final int l, final boolean convex) {
		boolean flag;
		int numericsHOri1, numericsHOri2;

		// if((triRef.inPointsList(i)==false)||(triRef.inPointsList(j)==false)||
		// (triRef.inPointsList(k)==false)||(triRef.inPointsList(l)==false))
		// System.out.println("Numerics.isInCone Not inPointsList " + i + " " +
		// j
		// + " " + k + " " + l);
		...
		if (convex) {
			if (i != j) {
				...
				// System.out.println("isInCone : i != j, numericsHOri1 = " +
				// numericsHOri1);
				if (numericsHOri1 < 0) {
					flag = false;
	 ...
	 */
	static int isConvexAngle(final Triangulator triRef, final int i,
			final int j, final int k, final int ind) {
		int angle;
		double numericsHDot;
		int numericsHOri1;
		Vector2f numericsHP, numericsHQ;

		// if((triRef.inPointsList(i)==false)||(triRef.inPointsList(j)==false)||
		// (triRef.inPointsList(k)==false))
		// System.out.println("Numerics.isConvexAngle: Not inPointsList " + i +
		// " " + j
		// + " " + k);
		...
		if (i == j) {
			...
			// System.out.println("i " + i + " j " + j + " k " + k + " ind " +
			// ind +
			...
			if (numericsHOri1 > 0) {
				...
				numericsHP = new Vector2f();
				numericsHQ = new Vector2f();
				Basic.vectorSub2D(triRef.points[i], triRef.points[j],
						numericsHP);
				Basic.vectorSub2D(triRef.points[k], triRef.points[j],
						numericsHQ);
	...
	/**
	 * This method checks whether point i4 is inside of or on the boundary of
	 * the triangle i1, i2, i3.
	 */
	static boolean pntInTriangle(final Triangulator triRef, final int i1,
	...
	/**
	 * This method checks whether point i4 is inside of or on the boundary of
	 * the triangle i1, i2, i3. it also returns a classification if i4 is on the
	 * boundary of the triangle (except for the edge i2, i3).
	 */
	static boolean vtxInTriangle(final Triangulator triRef, final int i1,
			final int i2, final int i3, final int i4, final int[] type) {
		boolean inside;
		int numericsHOri1;
	 ...
	 * intersection is reported if they intersect at a common vertex. the
	 * function assumes that i1 <= i2 and i3 <= i4. if i3 or i4 lies on i1, i2
	 * then an intersection is reported, but no intersection is reported if i1
	 * or i2 lies on i3, i4. this function is not symmetric!
	 */
	static boolean segIntersect(final Triangulator triRef, final int i1,
			final int i2, final int i3, final int i4, final int i5) {
		...
		// System.out.println("Numerics.segIntersect Not inPointsList " + i1 +
		// " " + i2
		// + " " + i3 + " " + i4);
		//
		// if((i1 > i2) || (i3 > i4))
		// System.out.println("Numerics.segIntersect i1>i2 or i3>i4 " + i1 + " "
		// + i2
		// + " " + i3 + " " + i4);
	...
	/**
	 * this function computes a quality measure of a triangle i, j, k. it
	 * returns the ratio `base / height', where base is the length of the
	 * longest side of the triangle, and height is the normal distance between
	 * the vertex opposite of the base side and the base side. (as usual, we
	 * again use the l1-norm for distances.)
	 */
	static double getRatio(final Triangulator triRef, final int i, final int j,
			final int k) {
		...
		// if((triRef.inPointsList(i)==false)||(triRef.inPointsList(j)==false)||
		// (triRef.inPointsList(k)==false))
		// System.out.println("Numerics.getRatio: Not inPointsList " + i + " " +
		// j
		// + " " + k);
	...
	static int spikeAngle(final Triangulator triRef, final int i, final int j,
			final int k, final int ind) {
		int ind1, ind2, ind3;

		// if((triRef.inPointsList(i)==false)||(triRef.inPointsList(j)==false)||
		// (triRef.inPointsList(k)==false))
		// System.out.println("Numerics.spikeAngle: Not inPointsList " + i + " "
		// + j
		// + " " + k);
	...
	static int recSpikeAngle(final Triangulator triRef, int i1, int i2, int i3,
			int ind1, int ind3) {
	 ...
	 * (this is no issue in our application, as we will always compute the angle
	 * centered at the mid-point of a valid diagonal.)
	 */
	static double angle(final Triangulator triRef, final Vector2f p,
			final Vector2f p1, final Vector2f p2) {
		int sign;
