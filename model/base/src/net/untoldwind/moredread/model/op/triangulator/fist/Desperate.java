...
class Desperate {
	 ...
	 * diagonals "valid".
	 */
	static boolean desperate(final Triangulator triRef, final int ind,
			final int i, final boolean[] splitted) {
		final int[] i1 = new int[1];
		final int[] i2 = new int[1];
		final int[] i3 = new int[1];
		final int[] i4 = new int[1];
		final int[] ind1 = new int[1];
		final int[] ind2 = new int[1];
		final int[] ind3 = new int[1];
		final int[] ind4 = new int[1];
		...
		if (existsSplit(triRef, ind, ind1, i1, ind2, i2)) {
			// break up the polygon by inserting this diagonal (which can't be
			// an
			// ear -- otherwise, we would not have ended up in this part of the
			// code). then, let's treat the two polygons separately. hopefully,
			// this will help to handle self-overlapping polygons in the
			// "correct"
			// way.
	...
	static boolean existsCrossOver(final Triangulator triRef, final int ind,
			final int[] ind1, final int[] i1, final int[] ind2, final int[] i2,
			final int[] ind3, final int[] i3, final int[] ind4, final int[] i4) {
		BBox bb1, bb2;
	...
	static void handleCrossOver(final Triangulator triRef, final int ind1,
			final int i1, final int ind2, final int i2, final int ind3,
			final int i3, final int ind4, final int i4) {
		...
		if (first) {
			...
			// StoreTriangle(GetOriginal(ind1), GetOriginal(ind2),
			// GetOriginal(ind3));
			...
			// StoreTriangle(GetOriginal(ind2), GetOriginal(ind3),
			// GetOriginal(ind4));
	...
	static boolean letsHope(final Triangulator triRef, final int ind) {
		int ind0, ind1, ind2;

		// let's clip the first convex corner. of course, we know that this is
		// no
	...
	static boolean existsSplit(final Triangulator triRef, final int ind,
			final int[] ind1, final int[] i1, final int[] ind2, final int[] i2) {
		int ind3, ind4, ind5;
		int i3, i4, i5;
	 ...
	 * point p. no care is taken to handle cases where p lies on the boundary of
	 * the polygon. (this is no issue in our application, as we will always
	 * compute the winding number with respect to the mid-point of a valid
	 * diagonal.)
	 */
	static int windingNumber(final Triangulator triRef, final int ind,
			final Vector2f p) {
		double angle;
		int ind2;
		int i1, i2, number;
	...
	static boolean foundSplit(final Triangulator triRef, int ind5, int i5,
			final int ind, final int ind1, final int i1, final int i3,
			final int i4, final int[] ind2, final int[] i2) {
		Vector2f center;
		int numDist = 0;
		int j, i6, i7;
		int ind6, ind7;
				...
				if (coneOk) {
					...
					if (coneOk) {
						...
						if (!NoHash.noHashEdgeIntersectionExists(triRef, bb,
								-1, -1, ind1, -1)) {
							// check whether this is a good diagonal; we do not
							// want a
							// diagonal that may create figure-8's!
							center = new Vector2f();
							Basic.vectorAdd2D(triRef.points[i1],
									triRef.points[i2[0]], center);
	...
	static void handleSplit(final Triangulator triRef, final int ind1,
			final int i1, final int ind3, final int i3) {
