...
public class MDKeyInput extends KeyInput implements KeyListener {
	private static final Logger logger = Logger.getLogger(MDKeyInput.class
			.getName());
	...
	@Override
	public int getKeyIndex(final String name) {
	...
	public void update() {
		if (listeners != null && listeners.size() > 0) {
			while (!events.isEmpty()) {
				final KeyEvent e = events.poll();
				final char c = e.character;
				final int keyCode = toInputCode(e.keyCode);
				final boolean pressed = (e.stateMask | SWT.KeyDown) == 1;

				for (int i = 0; i < listeners.size(); i++) {
					final KeyInputListener listener = listeners.get(i);
	 ...
	 */
	public static int toSWTCode(final int key) {
		switch (key) {
		case KEY_ESCAPE:
			return SWT.ESC;
		case KEY_1:
	 ...
	 */
	public static int toInputCode(final int key) {
		switch (key) {
		case SWT.ESC:
			return KEY_ESCAPE;
		case '1':
	...
	static String getKeyText(final int keyCode) {
		switch (keyCode) {
	...
	static String character(final char character) {
		switch (character) {
		case 0:
			return "'\\0'";
		case SWT.BS:
	...
	@Override
	public void clearKey(final int keycode) {
